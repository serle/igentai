...
async fn test_webserver_starts_in_test_mode() {
    ...
    assert!(
        webserver_result.is_ok(),
        "Webserver should not return an error in test mode"
    );
}
...
async fn test_webserver_fails_without_test_mode_and_no_orchestrator() {
    ...
    assert!(
        result.is_ok(),
        "Webserver should shut down gracefully even in offline mode"
    );

    let webserver_result = result.unwrap();
    assert!(
        webserver_result.is_ok(),
        "Webserver should continue in offline mode without crashing"
    );
}
