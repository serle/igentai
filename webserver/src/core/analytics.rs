...
use crate::types::{OptimizationInsight, InsightType, TrendDirection};
...
impl AnalyticsEngine {
    ...
    fn analyze_cost_efficiency(&self, 
        ...
        if let Some(avg_efficiency) = historical_efficiency {
            ...
            if relative_change < -0.15 { // Efficiency dropped by 15% or more
                Some(OptimizationInsight {
                    ...
                    recommendation: Some("Consider adjusting routing strategy or provider selection to improve cost efficiency".to_string()),
                    timestamp: Utc::now().timestamp() as u64,
                })
            } else if relative_change > 0.15 { // Efficiency improved significantly
                Some(OptimizationInsight {
                    insight_type: InsightType::CostEfficiency,
    ...
    fn analyze_performance_trends(&self, history: &[TimestampedMetrics]) -> Option<OptimizationInsight> {
        ...
        Some(OptimizationInsight {
            ...
            recommendation: match trend_direction {
                TrendDirection::Down => Some("Consider investigating recent changes or scaling up producers".to_string()),
                TrendDirection::Up => Some("Current strategy is working well, consider maintaining current settings".to_string()),
        ...
        for (provider_id, provider_metrics) in &current.by_provider {
            ...
            if provider_metrics.success_rate < 0.8 {
                insights.push(OptimizationInsight {
                    ...
                    title: format!("{:?} Provider Issues", provider_id),
                    description: format!(
                    ...
                    recommendation: Some(format!(
                        "Consider reducing load on {:?} or checking API key/rate limits", 
                        provider_id
                    )),
                    timestamp: Utc::now().timestamp() as u64,
                });
            ...
            // Check response time
            if provider_metrics.avg_response_time_ms > 10000.0 { // > 10 seconds
                insights.push(OptimizationInsight {
                    insight_type: InsightType::ProviderHealth,
                    title: format!("{:?} Slow Response", provider_id),
                    description: format!(
                    ...
                    recommendation: Some(format!(
                        "Consider reducing concurrent requests to {:?} or checking network connectivity", 
                        provider_id
                    )),
                    timestamp: Utc::now().timestamp() as u64,
                });
    ...
    fn analyze_routing_efficiency(&self, 
        current: &SystemMetrics,
        ...
        if coefficient_of_variation > 0.3 {
            Some(OptimizationInsight {
                ...
                recommendation: Some("Consider switching to weighted routing strategy to favor more efficient providers".to_string()),
                timestamp: Utc::now().timestamp() as u64,
            })
        } else {
            None
    ...
    fn calculate_average_efficiency(&self, history: &[TimestampedMetrics]) -> Option<f64> {
        ...
        let sum: f64 = history.iter()
            .map(|tm| tm.metrics.unique_per_dollar)
            .sum();
        ...
        F: Fn(&TimestampedMetrics) -> f64
    {
        ...
        let x_values: Vec<f64> = data.iter()
