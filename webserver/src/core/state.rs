...
use shared::{SystemMetrics, OrchestratorUpdate};
use shared::messages::webserver::CompletionReason;
use crate::types::{
    ClientSession, ClientMessage, DashboardData, 
    PerformanceMetrics, Alert, AlertLevel, ActivityEvent, ActivityType,
    OptimizationInsight, SystemHealth, TrendDirection, convert_orchestrator_update
};
...
impl WebServerState {
    ...
    fn get_provider_breakdown(&self) -> HashMap<shared::ProviderId, crate::types::ProviderStats> {
        if let Some(metrics) = &self.current_metrics {
            metrics.by_provider.iter().map(|(provider_id, provider_metrics)| {
                (*provider_id, crate::types::ProviderStats {
    ...
    fn get_producer_breakdown(&self) -> HashMap<shared::ProcessId, crate::types::ProducerStats> {
        if let Some(metrics) = &self.current_metrics {
            metrics.by_producer.iter().map(|(_producer_id, producer_metrics)| {
