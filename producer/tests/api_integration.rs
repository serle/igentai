...
use shared::{ProviderId, GenerationConfig};
use chrono::Utc;
...
async fn test_random_provider_api_integration() {
    ...
    let request = create_api_request(
        ProviderId::Random,
        "Generate 10 tourist attractions in Paris",
        100
    );
...
async fn test_random_provider_different_prompts() {
    ...
    for (prompt, max_tokens) in test_cases {
        ...
        assert!(!response.content.is_empty(), "Response should have content for prompt: {}", prompt);
        assert!(response.tokens_used <= max_tokens, "Token usage should respect limit for prompt: {}", prompt);
    }
}
...
async fn test_random_provider_concurrent_requests() {
    ...
    for i in 0..5 {
        ...
        let handle = tokio::spawn(async move {
            let request = create_api_request(
                ProviderId::Random,
                &format!("Generate {} places in Spain", i + 3),
                50
            );
...
async fn test_cost_estimation() {
    ...
    for (provider, tokens, expected_min_cost) in test_cases {
        ...
        assert!(cost >= expected_min_cost, 
               ...
               provider, tokens, expected_min_cost);
    }
}
...
async fn test_health_check_missing_keys() {
    ...
    let providers_needing_keys = vec![
        ProviderId::OpenAI,
    ...
    for provider in providers_needing_keys {
        ...
        assert!(!health_result.unwrap(), 
               "Health check for {:?} should return false without API key", provider);
    }
}
...
async fn test_request_with_different_generation_configs() {
    ...
    for (i, config) in configs.iter().enumerate() {
        ...
        assert!(response.tokens_used <= config.max_tokens, 
               "Token usage should respect config {} limit", i);
    }
}
...
async fn test_prompt_enhancement_integration() {
    use producer::core::{PromptHandler, Processor};
    ...
    assert!(enhanced_prompt.contains("alphanumeric"), "Prompt should include formatting requirements");
    assert!(enhanced_prompt.contains("lowercase"), "Prompt should specify lowercase requirement");
    ...
    assert!(!response.content.is_empty(), "Enhanced prompt response should have content");
}