...
async fn test_bloom_filter_serialization_roundtrip() {
    ...
    let serialized_bloom = serde_json::to_vec(&orchestrator_bloom)
    ...
    let seen_values: Vec<String> = orchestrator_data.iter()
        .map(|s| s.to_string())
        .collect();
    ...
    for item in &orchestrator_data {
        ...
        let stats = processor.process_response(response)
            .expect("Failed to process response");
        
        // Should detect as duplicate (bloom filter hit)
        assert_eq!(stats.new_values.len(), 0, 
                  "Item '{}' should be detected as duplicate", item);
        assert_eq!(stats.duplicate_count, 1, 
                  "Item '{}' should be counted as 1 duplicate", item);
    }
    ...
    for item in &new_items {
        ...
        assert_eq!(stats.new_values.len(), 1, 
                  "Item '{}' should be detected as new", item);
        assert_eq!(stats.duplicate_count, 0, 
                  "Item '{}' should not be counted as duplicate", item);
        assert_eq!(stats.new_values[0], *item, 
                  "New item should match input");
    }
...
async fn test_bloom_filter_large_dataset_e2e() {
    ...
    let orchestrator_data: Vec<String> = (0..5000)
    ...
    let serialized_bloom = serde_json::to_vec(&orchestrator_bloom)
        .expect("Failed to serialize large bloom filter");
...
async fn test_bloom_filter_false_positive_rate() {
    ...
    let orchestrator_data: Vec<String> = (0..dataset_size)
        .map(|i| format!("knownitem{}", i))
        .collect();
...
async fn test_bloom_filter_special_characters_e2e() {
    ...
    let special_data = vec![
        "café", "naïve", "résumé", "München", "北京",
        "🚀", "hello world", "test-item", "item.with.dots",
        "item_with_underscores", "UPPERCASE", "lowercase", "MiXeD_CaSe"
    ];
    ...
    let serialized_bloom = serde_json::to_vec(&orchestrator_bloom)
        .expect("Failed to serialize bloom filter with special characters");
    
    let seen_values: Vec<String> = special_data.iter()
        .map(|s| s.to_string())
        .collect();
    ...
    for item in &special_data {
        ...
        println!("📝 Item '{}': new={}, duplicates={}", 
                item, stats.new_values.len(), stats.duplicate_count);
    }
...
async fn test_bloom_filter_deserialization_error_handling() {
    ...
    assert_eq!(stats.duplicate_count, 1, "Should detect 'test1' as duplicate after fallback");
    ...
    let stats2 = processor.process_response(response2)
        .expect("Failed to process response with no bloom filter data");
    
    assert_eq!(stats2.duplicate_count, 1, "Should detect 'test2' as duplicate after None fallback");
