...
use crate::fixtures::{CommandFactory, ProcessIdFactory, TestProducerManager, UpdateFactory};
use shared::messages::producer::ProducerSyncStatus;
use shared::{ProcessStatus, ProcessId};
...
async fn test_orchestrator_multi_producer_commands() {
    ...
    let producer_ids: Vec<ProcessId> = (0..3).map(|_| ProcessIdFactory::create()).collect();
...
async fn test_multi_producer_bloom_sync() {
    ...
    let sync_commands: Vec<_> = producer_ids
        .iter()
        .enumerate()
        .map(|(i, _)| CommandFactory::sync_check_command((i + 1) as u64, Some(bloom_filter_data.clone()), true))
        .collect();
...
async fn test_multi_producer_error_handling() {
    ...
    let errors = vec![
        UpdateFactory::error(producer_ids[0].clone(), "API_ERROR", "OpenAI API key invalid", Some(1)),
        UpdateFactory::error(
            producer_ids[1].clone(),
            "NETWORK_ERROR",
