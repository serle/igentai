...
async fn test_start_command_handling() {
    ...
    let start_command = CommandFactory::start_command(
        1,
    ...
    match status_response {
        shared::ProducerUpdate::StatusUpdate { status, message, performance_stats, .. } => {
...
async fn test_update_config_command_handling() {
    ...
    // Test prompt update
    let config_update = CommandFactory::update_config_command(
        3,
        Some("Generate Italian pasta dishes".to_string()),
    );
    
    match &config_update {
        ProducerCommand::UpdateConfig { command_id, prompt, routing_strategy, generation_config } => {
...
async fn test_sync_check_command_handling() {
    ...
    let sync_command = CommandFactory::sync_check_command(
        4,
        Some(bloom_data.clone()),
        true,
    );
    ...
    match success_response {
        shared::ProducerUpdate::SyncAck { sync_id, status, bloom_version, .. } => {
            assert_eq!(sync_id, 4);
            assert_eq!(bloom_version, Some(1));
            match status {
    ...
    let busy_response = UpdateFactory::sync_ack(
        producer_id.clone(),
        4,
    ...
    let failed_response = UpdateFactory::sync_ack(
        producer_id,
        4,
        ProducerSyncStatus::Failed { reason: "Invalid bloom filter format".to_string() },
    );
...
async fn test_invalid_command_handling() {
    ...
    for (error_code, message, command_id) in error_scenarios {
        let error_response = UpdateFactory::error(
            producer_id.clone(),
            error_code,
            message,
            command_id,
        ...
        match error_response {
            shared::ProducerUpdate::Error { error_code: code, message: msg, command_id: cmd_id, .. } => {
...
async fn test_attribute_batch_processing() {
    ...
    for (batch_id, provider) in providers.iter().enumerate() {
        let attributes = (0..10)
            .map(|i| format!("attribute_{}_{}", batch_id, i))
            .collect();
