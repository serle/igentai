...
use shared::messages::producer::{ProducerPerformanceStats, ProducerSyncStatus, ProviderUsageStats};
...
async fn test_bloom_filter_sync_workflow() {
    ...
    let _initial_ack = UpdateFactory::sync_ack(producer_id.clone(), 1, ProducerSyncStatus::Ready);
...
async fn test_error_propagation() {
    ...
    for (error_code, error_message, command_id) in error_scenarios {
        let error_update = UpdateFactory::error(producer_id.clone(), error_code, error_message, command_id);
