...
impl TestProducer {
    ...
    pub async fn start(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        if self.test_mode {
            // Use topic and random provider for CLI mode
            cmd.arg("--topic").arg(&self.topic);
            cmd.arg("--provider").arg("random");
        } else {
            // For production mode, use orchestrator address
            cmd.arg("--orchestrator-addr")
                .arg(format!("{}:{}", self.listen_addr.ip(), self.listen_addr.port()));
        }
    ...
    pub async fn stop(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        if let Some(mut child) = self.child.take() {
            ...
            if let Ok(result) = timeout(Duration::from_secs(5), async { child.wait() }).await {
                result?;
            } else {
...
impl TestProducerManager {
    ...
    pub async fn spawn_producer(
        ...
        topic: String,
        test_mode: bool,
    ) -> Result<ProcessId, Box<dyn std::error::Error + Send + Sync>> {
        use std::sync::atomic::{AtomicU32, Ordering};
        static COUNTER: AtomicU32 = AtomicU32::new(1);
        let producer_id = ProcessId::Producer(COUNTER.fetch_add(1, Ordering::SeqCst));
        ...
        let mut producer = TestProducer::new(producer_id.clone(), listen_addr, command_addr, topic, test_mode);
    ...
    pub async fn get_producer_ids(&self) -> Vec<ProcessId> {
    ...
    pub async fn get_producer_addresses(&self, producer_id: &ProcessId) -> Option<(SocketAddr, SocketAddr)> {
        let producers = self.producers.read().await;
        producers
            .iter()
