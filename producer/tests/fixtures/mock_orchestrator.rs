...
use shared::{ProducerCommand, ProducerUpdate, ProducerId};
...
pub struct MockOrchestrator {
    ...
    producer_streams: Arc<RwLock<HashMap<ProducerId, TcpStream>>>,
...
impl MockOrchestrator {
    ...
    async fn handle_producer_connection(
        ...
        let mut producer_id_opt: Option<ProducerId> = None;
    ...
    pub async fn get_connected_producers(&self) -> Vec<ProducerId> {
        self.producer_streams.read().await.keys().cloned().collect()
    }
    
    /// Send command to specific producer (if connected)
    pub async fn send_command_to_producer(&self, producer_id: &ProducerId, command: ProducerCommand) -> bool {
