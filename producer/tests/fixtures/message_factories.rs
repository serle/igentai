...
use shared::{
    ProducerCommand, ProducerUpdate, ProducerCommand as OrchestratorCommand,
    ProducerId, ProviderMetadata, ProcessStatus, RoutingStrategy, GenerationConfig,
...
impl UpdateFactory {
    ...
    pub fn pong(producer_id: ProducerId, ping_id: u64) -> ProducerUpdate {
        ProducerUpdate::Pong {
            producer_id,
            ping_id,
        }
...
/// Factory for creating test producer IDs
pub struct ProducerIdFactory;

#[allow(dead_code)] // Utility methods may not all be used
impl ProducerIdFactory {
    /// Create a test producer ID
    pub fn create() -> ProducerId {
        shared::types::ProducerId::from_uuid(Uuid::new_v4())
    }
    
    /// Create a producer ID from a specific UUID
    pub fn from_uuid(uuid: Uuid) -> ProducerId {
        shared::types::ProducerId::from_uuid(uuid)
    }
}
...
mod tests {
    ...
    fn test_command_factory() {
        ...
        match start_cmd {
            OrchestratorCommand::Start { command_id, topic, prompt, .. } => {
    ...
    fn test_update_factory() {
        ...
        let batch_update = UpdateFactory::attribute_batch(
            producer_id.clone(),
            1,
            attributes.clone(),
            ProviderIdEnum::OpenAI,
        );
        
        match batch_update {
            ProducerUpdate::AttributeBatch { batch_id, attributes: attrs, .. } => {
        ...
        match status_update {
            ProducerUpdate::StatusUpdate { status, message, performance_stats, .. } => {
