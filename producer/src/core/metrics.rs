...
impl Metrics {
    ...
    pub fn record_response_received(&mut self, response: &ApiResponse) {
        ...
        debug!("Recorded response from {:?}: {}ms, {} tokens, success: {}", 
    ...
    pub fn record_attributes_processed(&mut self, attributes: &[ProcessedAttribute]) {
        ...
        debug!("Recorded {} attributes processed ({} unique)", attributes.len(), unique_count);
        self.update_timestamp();
    }
    ...
    pub fn record_processing_stats(&mut self, stats: &crate::core::processor::ProcessingStats) {
        ...
        debug!("Recorded {} attributes processed ({} new, {} duplicates)", 
               stats.total_extracted, stats.new_values.len(), stats.duplicate_count);
        self.update_timestamp();
    }
    ...
    pub fn get_provider_metrics(&self) -> HashMap<ProviderId, ProviderMetrics> {
        self.provider_stats.iter().map(|(provider, stats)| {
    ...
    pub fn get_insights(&self) -> Vec<PerformanceInsight> {
        ...
        if self.current_metrics.avg_response_time_ms > 5000.0 {
            insights.push(PerformanceInsight {
                ...
                message: format!("Average response time is high: {:.0}ms", self.current_metrics.avg_response_time_ms),
                severity: InsightSeverity::Warning,
            });
        }
        ...
        if efficiency < 10.0 && self.current_metrics.total_cost > 1.0 {
            insights.push(PerformanceInsight {
                ...
                message: format!("Cost efficiency is low: {:.1} attributes per dollar", efficiency),
        ...
        for (provider, stats) in &self.provider_stats {
            if stats.success_rate() < 0.7 && stats.requests_sent > 10 {
                insights.push(PerformanceInsight {
                    ...
                    message: format!("{:?} has low success rate: {:.1}%", provider, stats.success_rate() * 100.0),
                    severity: InsightSeverity::Warning,
                });
            }
            ...
            if stats.avg_response_time_ms() > 8000.0 && stats.responses_received > 5 {
                insights.push(PerformanceInsight {
                    category: InsightCategory::Provider,
                    message: format!("{:?} has slow response times: {:.0}ms", provider, stats.avg_response_time_ms()),
                    severity: InsightSeverity::Info,
                });
            }
        }
    ...
    fn recalculate_derived_metrics(&mut self) {
        ...
        if !self.response_times.is_empty() {
            let total_ms: u64 = self.response_times.iter()
...
mod tests {
    ...
    fn test_performance_insights() {
        ...
        let reliability_insights: Vec<_> = insights.iter()
