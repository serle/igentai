...
impl PromptHandler {
    ...
    pub fn new() -> Self {
        ...
        provider_limits.insert(ProviderId::OpenAI, ProviderLimits {
        ...
        provider_limits.insert(ProviderId::Anthropic, ProviderLimits {
        ...
        provider_limits.insert(ProviderId::Gemini, ProviderLimits {
        ...
        provider_limits.insert(ProviderId::Random, ProviderLimits {
            context_window: 8192,
    ...
    pub fn calculate_optimal_exclusions(
        ...
        let limits = self.provider_limits.get(&provider)
        ...
        let available_input_tokens = limits.context_window
    ...
    pub async fn build_enhanced_prompt(
        ...
        let (existing_entries, bloom_info) = if combined_seen_values.is_empty() {
            ...
            let recent_entries: Vec<String> = combined_seen_values.iter()
                .take(optimal_exclusions)
                .cloned()
                .collect();
            
            let exclusion_info = if combined_seen_values.len() > optimal_exclusions {
                format!("(showing {} most recent of {} total)", optimal_exclusions, combined_seen_values.len())
            } else {
        ...
        let request_size = generation_config
            .map(|gc| gc.request_size)
            .unwrap_or(100);
        
        // Build enhanced prompt with provider-optimized exclusion template
        let enhanced_prompt = format!(r#"Generate {} new entries about: {}
...
- Do NOT repeat any previously seen entries, even with slight variations"#, 
            request_size, base_prompt, existing_entries, bloom_info);
    ...
    pub fn estimate_tokens(&self, text: &str, provider: ProviderId) -> u32 {
        ...
        let tokens_per_word = self.provider_limits
            .get(&provider)
