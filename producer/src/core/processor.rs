...
impl Processor {
    ...
    pub fn process_response(&mut self, response: ApiResponse) -> ProducerResult<ProcessingStats> {
        ...
        debug!("Processed {:?} response: {} total, {} new, {} duplicates", 
               response.provider, stats.total_extracted, stats.new_values.len(), stats.duplicate_count);
    ...
    pub fn update_bloom_filter(&mut self, bloom_data: Option<Vec<u8>>, values: Vec<String>) {
        ...
        if let Some(data) = bloom_data {
            ...
            info!("📋 No bloom filter provided, rebuilding from {} values", self.seen_values.len());
            self.bloom_filter = Self::build_bloom_filter(&self.seen_values);
        }
    }
    ...
    fn deserialize_bloom_filter(data: &[u8]) -> Result<GrowableBloom, String> {
        serde_json::from_slice::<GrowableBloom>(data)
            .map_err(|e| format!("Failed to deserialize bloom filter: {}", e))
    }
...
mod tests {
    ...
    fn test_simple_word_extraction() {
        ...
        let response = create_test_response(
            ProviderId::OpenAI, 
            "red blue green yellow orange purple".to_string()
        );
    ...
    fn test_bloom_filter_update() {
        ...
        let response = create_test_response(
            ProviderId::OpenAI,
            "test new example fresh".to_string()
        );
        let stats = processor.process_response(response).unwrap();
