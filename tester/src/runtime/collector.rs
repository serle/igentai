...
impl TracingCollector {
    ...
    pub fn query(&self, query: &TraceQuery) -> Vec<CollectedEvent> {
        ...
        let cutoff_time = query.since_seconds_ago.map(|seconds| {
        ...
        tracing::debug!("🔍 Query completed in {:?}, returned {} events", query_time, filtered.len());
    ...
    pub fn get_topic_trace_subset(&self, topic: &str) -> Vec<CollectedEvent> {
        ...
        tracing::info!("📊 Extracted {} trace events for topic '{}' (completed: {})", 
                      topic_events.len(), topic, topic_completed);
        topic_events
    }
