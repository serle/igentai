...
use crate::{
    runtime::{CollectedEvent, TracingCollector},
    testing::{OutputData, OutputLoader, TracingAssertions},
};
...
impl Topic {
    ...
    pub async fn wait_for_topic(topic_name: &str, collector: TracingCollector, timeout: Duration) -> Option<Self> {
        ...
        tracing::info!(
            "✅ Topic '{}' ready for testing ({} trace events)",
            topic_name,
            trace_events.len()
        );
    ...
    pub async fn wait_for_topic_indefinitely(topic_name: &str, collector: TracingCollector) -> Option<Self> {
        tracing::info!(
            "🔍 Waiting indefinitely for topic '{}' to complete (WebServer mode)",
            topic_name
        );
            ...
            if !trace_events.is_empty() {
                ...
                let has_completion = trace_events.iter().any(|event| {
                    event
                        .trace_event
                        .message
                        .contains(&format!("✅ Topic '{}' completed after", topic_name))
                });
    ...
    pub async fn assert_process_logged(&self, process: &str, message: &str) -> bool {
        let result = self
            .assertions
            .assert_process_logged(process, message, Duration::from_secs(10))
            .await;
        if result.success {
    ...
    pub fn assert_attributes_matching(&self, pattern: &str, min_count: usize) -> bool {
        if let Some(output) = &self.output_data {
            ...
            if matching.len() >= min_count {
                tracing::info!(
                    "✅ Found {} attributes matching '{}' (>= {} required)",
                    matching.len(),
                    pattern,
                    min_count
                );
    ...
    pub fn print_summary(&self) {
        ...
        if !self.trace_events.is_empty() {
            ...
            tracing::info!(
                ...
                last_event
                    .received_at
                    .duration_since(first_event.received_at)
                    .unwrap_or_default()
            );
        }
    }
    ...
    pub fn print_recent_traces(&self, count: usize) {
        tracing::info!(
            "📋 Recent {} trace events for '{}':",
            count.min(self.trace_events.len()),
            self.name
        );
