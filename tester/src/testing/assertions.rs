...
use crate::runtime::{TraceQuery, TracingCollector};
...
impl TracingAssertions {
    ...
    pub async fn assert_process_logged(
        ...
        tracing::info!(
            "🔍 Asserting {} logged message containing: '{}'",
            process,
            message_contains
        );
        ...
        if found {
            ...
            AssertionResult::failure(
                format!(
                    "Process '{}' did not log message containing '{}' within timeout",
                    process, message_contains
                ),
    ...
    pub async fn assert_coordination_sequence(
        ...
        for (process, message_fragment) in &sequence {
            ...
            if found {
                ...
                return AssertionResult::failure(
                    format!(
                        "Coordination sequence failed at step: {} should log '{}'",
                        process, message_fragment
                    ),
        ...
        AssertionResult::success(
            format!(
                "Coordination sequence completed successfully ({} steps)",
                sequence.len()
            ),
            found_sequence.len(),
        )
    ...
    pub async fn assert_all_processes_active(
        ...
        if active_processes.len() == expected_processes.len() {
            AssertionResult::success("All expected processes are active".to_string(), active_processes.len())
        } else {
    ...
    pub async fn assert_trace_propagation(&self, expected_processes: Vec<&str>, _timeout: Duration) -> AssertionResult {
        tracing::info!(
            "📡 Asserting trace propagation across {} processes",
            expected_processes.len()
        );
        ...
        if processes_with_tracing.len() == expected_processes.len() {
            ...
            AssertionResult::failure(
                format!(
                    "Not all processes have tracing configured ({}/{})",
                    processes_with_tracing.len(),
                    expected_processes.len()
                ),
    ...
    pub async fn assert_producer_generation_workflow(
        ...
        tracing::info!(
            "🏭 Asserting producer generation workflow for {} producers",
            producer_count
        );
        ...
        AssertionResult::success(
            format!(
                "Producer generation workflow completed ({} steps)",
                workflow_steps.len()
            ),
            completed_steps,
        )
    ...
    pub async fn assert_no_errors(&self, since_seconds_ago: u64) -> AssertionResult {
        tracing::info!(
            "🚫 Asserting no error messages in the last {} seconds",
            since_seconds_ago
        );
        ...
        if error_events.is_empty() {
            AssertionResult::success("No error messages found".to_string(), 0)
        } else {
    ...
    pub async fn assert_event_frequency(
        ...
        tracing::info!(
            "📊 Asserting event frequency: {} should log '{}' at least {} times in {}s",
            process,
            message_contains,
            expected_min_count,
            time_window_seconds
        ...
        if events.len() >= expected_min_count {
            AssertionResult::success(
                format!(
                    "Event frequency assertion passed: found {} events (expected min: {})",
                    events.len(),
                    expected_min_count
                ),
                events.len(),
            ...
            AssertionResult::failure(
                format!(
                    "Event frequency assertion failed: found {} events (expected min: {})",
                    events.len(),
                    expected_min_count
                ),
                Some(format!(
                    "Time window: {}s, Process: {}, Message: '{}'",
    ...
    pub async fn assert_topic_completed(&self, topic: &str, timeout: Duration) -> AssertionResult {
        ...
        if self.collector.wait_for_topic_completion(topic, timeout).await {
            ...
            if let Some(event) = completion_events.first() {
                AssertionResult::success(format!("Topic '{}' completed successfully", topic), 1)
