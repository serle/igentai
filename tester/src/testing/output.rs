...
impl OutputLoader {
    ...
    pub fn load_output_file<P: AsRef<Path>>(path: P) -> Result<OutputData, Box<dyn std::error::Error>> {
        ...
        tracing::info!("📄 Loaded {} attributes from {}", attributes.len(), file_path);
        ...
        if metadata_file.exists() {
            match Self::load_metadata(&metadata_file) {
                Ok(metadata) => {
                    ...
                    tracing::info!("📊 Loaded metadata from {}", metadata_file.display());
                }
                Err(e) => {
                    tracing::warn!("⚠️ Failed to load metadata: {}", e);
                }
            }
        }
    ...
    pub fn load_from_traces(collector: &TracingCollector) -> Result<OutputData, Box<dyn std::error::Error>> {
        let topic = collector.extract_topic().ok_or("Could not extract topic from traces")?;

        let output_dir = format!("./output/{}", topic);
        tracing::info!("📂 Loading output from extracted topic '{}' -> {}", topic, output_dir);
    ...
    pub fn load_from_topic(topic: &str) -> Result<OutputData, Box<dyn std::error::Error>> {
        let output_dir = format!("./output/{}", topic);
        tracing::info!("📂 Loading output for topic '{}' -> {}", topic, output_dir);

        Self::load_output_dir(&output_dir)
    }

    /// Load output with custom base directory and extracted topic
    pub fn load_from_traces_with_base_dir(
...
impl OutputData {
    ...
    pub fn find_attributes_containing(&self, substring: &str) -> Vec<&String> {
        self.attributes.iter().filter(|attr| attr.contains(substring)).collect()
    }
