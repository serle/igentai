...
use shared::{
    ProcessId, WebServerRequest, OrchestratorUpdate, OrchestratorCommand, ProducerUpdate,
    OptimizationMode, GenerationConstraints, ProcessStatus, logging, process_info, process_debug, process_error
};
    ...
    P: ProcessManager + Send + Sync + 'static,
{
    ...
    pub async fn initialize(&mut self, 
        ...
        process_debug!(ProcessId::current(), "🌍 WebServer spawned and registered at {}", webserver_info.api_address);
    ...
    pub async fn initialize_cli_mode(&mut self, 
        producer_bind_addr: std::net::SocketAddr,
    ) -> OrchestratorResult<()> {
    ...
    pub async fn start_cli_generation(&mut self, topic: String, producer_count: u32, iterations: Option<u32>, request_size: usize) -> OrchestratorResult<()> {
        ...
        let budget_str = match iterations {
            Some(limit) => format!("with {} iteration budget", limit),
        ...
        self.start_generation_with_config(1, topic, producer_count, optimization_mode, constraints, request_size).await?;
    ...
    async fn handle_webserver_request(&self, request: WebServerRequest) -> OrchestratorResult<()> {
        match request {
            WebServerRequest::StartGeneration { request_id, topic, producer_count, optimization_mode, constraints, iterations } => {
                ...
                self.start_generation(request_id, topic, producer_count, optimization_mode, constraints).await
            },
            
            WebServerRequest::StopGeneration { request_id } => {
                self.stop_generation(request_id).await
            },
            
            WebServerRequest::GetStatus { request_id } => {
                self.send_status_update(request_id).await
            },
            
            WebServerRequest::UpdateConfig { request_id, optimization_mode, constraints } => {
    ...
    async fn handle_webserver_ready(&self, listen_port: u16, http_port: u16) -> OrchestratorResult<()> {
        ...
        process_debug!(ProcessId::current(), "🚀 WebServer ready: HTTP on port {}, IPC on port {}", http_port, listen_port);
    ...
    async fn handle_producer_ready(&self, producer_id: ProcessId, listen_port: u16) -> OrchestratorResult<()> {
        ...
        self.communicator.mark_producer_ready(producer_id.clone(), producer_addr).await?;
        
        process_debug!(ProcessId::current(), "🚀 Producer {} ready on port {}", producer_id, listen_port);
        ...
        if let Some(command) = pending_command {
            process_debug!(ProcessId::current(), "📤 Sending queued start command to producer {}", producer_id);
    ...
    async fn handle_producer_update(&self, update: ProducerUpdate) -> OrchestratorResult<()> {
        match update {
            ProducerUpdate::AttributeBatch { producer_id, batch_id: _, attributes, provider_metadata } => {
                self.process_attribute_batch(producer_id, attributes, provider_metadata).await
            },
            
            ProducerUpdate::SyncAck { producer_id, sync_id: _, bloom_version: _, status: _ } => {
                // Process sync acknowledgment
                self.handle_sync_acknowledgment(producer_id).await
            },
            
            ProducerUpdate::StatusUpdate { producer_id, status, message: _, performance_stats: _ } => {
                self.update_producer_status(producer_id, status).await
            },
            
            ProducerUpdate::Pong { producer_id, ping_id: _ } => {
                self.handle_pong(producer_id).await
            },
            
            ProducerUpdate::Error { producer_id, error_code, message, command_id: _ } => {
                self.handle_producer_error(producer_id, error_code, message).await
            },
            
            ProducerUpdate::Ready { producer_id, listen_port } => {
    ...
    async fn start_generation_with_config(
        ...
        let producer_infos = self.process_manager.spawn_producers(producer_count, &topic, api_keys.clone(), producer_addr).await?;
        
        // Register producers with communicator
        for info in &producer_infos {
            self.communicator.register_producer(info.id.clone(), info.command_address).await?;
        }
        ...
        let (prompt, mut routing_strategy, mut generation_config) = {
            ...
            let optimization = optimizer.optimize_for_topic(&topic, &state.get_performance_stats(), &state.context.optimization_targets.optimization_mode)?;
    ...
    async fn start_generation(
        ...
        process_info!(ProcessId::current(), "✅ Topic '{}' started with no iteration budget", topic);
        ...
        // Spawn producers
        let producer_addr = self.producer_addr.expect("Producer address not initialized");
        let producer_infos = self.process_manager.spawn_producers(producer_count, &topic, api_keys, producer_addr).await?;
        ...
        let (prompt, routing_strategy, generation_config) = {
            ...
            (optimization.optimized_prompt, optimization.routing_strategy, optimization.generation_config)
        };
    ...
    async fn stop_generation(&self, request_id: u64) -> OrchestratorResult<()> {
        ...
        // Update state and send completion notification
        {
            ...
            if let Some(topic) = &state.context.topic {
                ...
                process_info!(ProcessId::current(), "✅ Topic '{}' completed after {} iterations", topic, current_iteration);
    ...
    async fn process_attribute_batch(
        ...
        process_debug!(ProcessId::current(), "📥 Received {} attributes from producer {}", attributes.len(), producer_id);
        ...
        let unique_attributes = {
            ...
            if let Some(topic) = &state.context.topic {
                self.file_system.write_unique_attributes_with_metadata(topic, &unique_attrs, &provider_metadata).await?;
            }
        ...
        if !unique_attributes.is_empty() {
            process_debug!(ProcessId::current(), "📤 Sending {} unique attributes to webserver", unique_attributes.len());
        ...
        process_debug!(ProcessId::current(), "🏁 CLI mode: Iteration limit reached, shutting down");
    ...
    async fn update_producer_status(&self, producer_id: ProcessId, status: ProcessStatus) -> OrchestratorResult<()> {
        ...
        if status == ProcessStatus::Running {
            if let Some(topic) = &state.context.topic {
                ...
                let optimizer = state.get_optimizer();
                if let Ok(optimization) = optimizer.optimize_for_topic(topic, &state.get_performance_stats(), &state.context.optimization_targets.optimization_mode) {
                    let command = OrchestratorCommand::Start {
                    ...
                    process_debug!(ProcessId::current(), "📋 Sending start command to producer {} for topic '{}'", producer_id, topic);
                    
                    // Drop the state lock before async call
                    drop(state);
                    
                    // Send the command directly
                    if let Err(e) = self.communicator.send_producer_command(producer_id.clone(), command).await {
                        process_error!(ProcessId::current(), "❌ Failed to send start command to producer {}: {}", producer_id, e);
                    } else {
                        process_debug!(ProcessId::current(), "✅ Start command sent to producer {}", producer_id);
                    }
    ...
    async fn handle_producer_error(&self, producer_id: ProcessId, error_code: String, message: String) -> OrchestratorResult<()> {
        process_error!(ProcessId::current(), "❌ Producer {} error [{}]: {}", producer_id, error_code, message);
    ...
    async fn collect_and_send_metrics(&self) -> OrchestratorResult<()> {
        let (metrics, active_producers, current_topic, total_unique) = {
            ...
            if let Some(topic) = &state.context.topic {
                if let Ok(optimization) = optimizer.optimize_for_topic(topic, &performance_stats, &state.context.optimization_targets.optimization_mode) {
                    if optimization.confidence > 0.8 {
                        process_debug!(ProcessId::current(), "🎯 Optimization opportunity found (confidence: {:.1}%): {}", 
                                optimization.confidence * 100.0, optimization.rationale);
    ...
    async fn perform_health_checks(&self) -> OrchestratorResult<()> {
        ...
        if !failed_producers.is_empty() {
            process_info!(ProcessId::current(), "🔄 Healing producer pool: restarting {} failed producers", failed_producers.len());
            
            for failed_producer_id in &failed_producers {
                if let Err(e) = self.restart_failed_producer(failed_producer_id.clone()).await {
                    process_error!(ProcessId::current(), "❌ Failed to restart producer {}: {}", failed_producer_id, e);
                } else {
                    process_info!(ProcessId::current(), "✅ Producer {} successfully restarted", failed_producer_id);
                }
            }
            
            // Send error notification to webserver (if it exists)
            if self.webserver_rx.is_some() {
                let status_update = OrchestratorUpdate::ErrorNotification(
                    format!("{} producers failed and were restarted", failed_producers.len())
                );
    ...
    async fn restart_failed_producer(&self, failed_producer_id: ProcessId) -> OrchestratorResult<()> {
        ...
        if let Some(topic) = topic {
            ...
            let producer_addr = self.producer_addr.expect("Producer address not initialized");
            let producer_infos = self.process_manager.spawn_producers(1, &topic, api_keys, producer_addr).await?;
            
            if let Some(new_producer_info) = producer_infos.first() {
                // Register the new producer
                self.communicator.register_producer(new_producer_info.id.clone(), new_producer_info.command_address).await?;
                
                // Update state: remove old producer and add new one
                {
                    let mut state = self.state.lock().await;
                    state.remove_producer(&failed_producer_id);
                    state.add_producer(new_producer_info.id.clone(), new_producer_info.process_id, ProcessStatus::Running);
                ...
                process_debug!(ProcessId::current(), "🏭 Spawned replacement producer {} for failed producer {}", new_producer_info.id, failed_producer_id);
            }
        }
