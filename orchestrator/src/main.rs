...
use shared::{ProcessId, logging, process_info, process_debug};
...
async fn main() -> OrchestratorResult<()> {
    ...
    if cli_mode {
        ...
        process_debug!(ProcessId::current(), "Topic: {}, Producers: {}, Iterations: {}", 
            topic,
            args.producers,
            args.iterations.map(|i| i.to_string()).unwrap_or_else(|| "unlimited".to_string()));
        process_debug!(ProcessId::current(), "Provider: {}, Request Size: {}, Output: {}", 
            if use_only_random { "random (Random)" } else { "env (API keys)" },
            args.request_size,
            output_dir);
    } else {
        logging::log_startup(ProcessId::current(), "orchestrator service (web mode)");
        if use_only_random {
            process_debug!(ProcessId::current(), "🎲 Random provider mode enabled - using dummy API keys");
        }
    }
    ...
    if cli_mode {
        ...
        orchestrator.start_cli_generation(topic, args.producers, args.iterations, args.request_size).await?;
    } else {
