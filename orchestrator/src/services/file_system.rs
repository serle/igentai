...
impl FileSystem for RealFileSystem {
    async fn create_topic_directory(&self, topic: &str) -> OrchestratorResult<()> {
        ...
        let metadata_content =
            serde_json::to_string_pretty(&metadata).map_err(|e| OrchestratorError::JsonError { source: e })?;
    ...
    async fn write_unique_attributes(&self, topic: &str, attributes: &[String]) -> OrchestratorResult<()> {
        ...
        for attribute in attributes {
            ...
            let line = serde_json::to_string(&entry).map_err(|e| OrchestratorError::JsonError { source: e })?;
        ...
        process_debug!(
            shared::ProcessId::current(),
            "💾 Wrote {} unique attributes for topic '{}'",
            attributes.len(),
            topic
        );
        Ok(())
    ...
    async fn write_unique_attributes_with_metadata(
        ...
        process_debug!(
            shared::ProcessId::current(),
            "💾 Wrote {} unique attributes with provider metadata for topic '{}'",
            attributes.len(),
            topic
        );
        Ok(())
    ...
    async fn read_topic_attributes(&self, topic: &str) -> OrchestratorResult<Vec<String>> {
        ...
        for line in content.lines() {
            ...
            let entry: serde_json::Value =
                serde_json::from_str(line).map_err(|e| OrchestratorError::JsonError { source: e })?;
