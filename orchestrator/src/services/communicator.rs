...
impl RealCommunicator {
    ...
    async fn send<T: serde::Serialize>(&self, address: SocketAddr, message: &T) -> OrchestratorResult<()> {
        match TcpStream::connect(address).await {
            ...
            Err(e) => Err(OrchestratorError::communication(format!(
                "Connect failed to {address}: {e}"
            ))),
        }
    }

    /// Start TCP listener for incoming messages
    async fn listen<T, F>(&self, bind_addr: SocketAddr, tx: mpsc::Sender<T>, parser: F) -> OrchestratorResult<()>
    where
        T: Send + 'static,
    ...
    async fn send_producer_command(
        ...
        if ready {
            if let Some(addr) = address {
                ...
                process_debug!(
                    shared::ProcessId::current(),
                    "📤 Sent command to producer {}: {:?}",
                    producer_id,
                    command
                );
            } else {
                return Err(OrchestratorError::communication(format!(
                    "Producer {producer_id} not registered"
                )));
            }
        } else {
            ...
            process_debug!(
                shared::ProcessId::current(),
                "⏳ Producer {} not ready yet, ignoring command",
                producer_id
            );
        }
