...
use shared::{ProviderId, SystemMetrics, ProcessId, process_info, process_debug, OrchestratorCommand};
use crate::error::OrchestratorResult;
use super::{UniquenessTracker, PerformanceTracker, Optimizer};
...
impl OrchestratorState {
    ...
    pub fn get_system_metrics(&self) -> SystemMetrics {
        ...
        SystemMetrics {
            ...
            by_producer: performance_stats.by_producer.iter()
                .map(|(id, metrics)| (id.to_string(), convert_to_producer_metrics(metrics.clone())))
                .collect(),
            by_provider: performance_stats.by_provider.iter()
    ...
    pub fn get_producers_needing_sync(&self) -> Vec<ProcessId> {
        ...
        self.producers.iter()
            .filter(|(_, state)| {
                state.last_sync_version.map_or(true, |v| v < current_version)
            })
            .map(|(id, _)| id.clone())
            .collect()
    }
    ...
    pub fn increment_iteration(&mut self) -> bool {
        ...
        process_debug!(ProcessId::current(), "   • Cost/Minute: ${:.4}", metrics.cost_per_minute);
        process_debug!(ProcessId::current(), "   • Total Unique Attributes: {}", current_unique_count);
        ...
        process_debug!(ProcessId::current(), "");
        process_debug!(ProcessId::current(), "🎯 New Unique Attributes This Iteration ({}):", new_values);
        ...
        if let Some(limit) = self.cli_iterations {
            if self.current_iteration >= limit {
                process_debug!(ProcessId::current(), "🏁 Reached iteration limit: {}/{}", self.current_iteration, limit);
    ...
    pub fn add_attributes(&mut self, attributes: Vec<String>, provider_metadata: &shared::ProviderMetadata) -> Vec<String> {
    ...
    pub fn generate_cycle_performance_summary(&self) -> Option<CyclePerformanceSummary> {
        ...
        let average_efficiency = self.cycle_history.iter()
            .map(|c| c.efficiency)
            .sum::<f64>() / self.cycle_history.len() as f64;
        let peak_efficiency = self.cycle_history.iter()
            .map(|c| c.efficiency)
            .fold(0.0, f64::max);
        ...
        let efficiency_decline_rate = if self.cycle_history.len() >= 2 {
            ...
            let sum_xy = self.cycle_history.iter().enumerate()
                .map(|(i, c)| ((i + 1) as f64) * c.efficiency)
                .sum::<f64>();
            let sum_x2 = (1..=self.cycle_history.len())
                .map(|i| (i * i) as f64)
                .sum::<f64>();
    ...
    pub fn generate_provider_performance_stats(&self) -> Vec<ProviderPerformanceStats> {
        ...
        performance_stats.by_provider.iter().map(|(provider_id, metrics)| {
    ...
    pub async fn export_cycle_performance(&self, file_system: &dyn crate::traits::FileSystem) -> OrchestratorResult<()> {
        if let Some(summary) = self.generate_cycle_performance_summary() {
            let json_content = serde_json::to_string_pretty(&summary)?;
            
            file_system.write_file("cycle_performance.json", json_content.as_bytes()).await?;
            process_debug!(ProcessId::current(), "📊 Exported cycle performance to cycle_performance.json");
        }
        Ok(())
    }
    
    /// Export provider performance to JSON file
    pub async fn export_provider_performance(&self, file_system: &dyn crate::traits::FileSystem) -> OrchestratorResult<()> {
        let provider_stats = self.generate_provider_performance_stats();
        let json_content = serde_json::to_string_pretty(&provider_stats)?;
        
        file_system.write_file("provider_performance.json", json_content.as_bytes()).await?;
        process_debug!(ProcessId::current(), "📊 Exported provider performance to provider_performance.json");
        Ok(())
    }
}
...
impl OrchestratorState {
    ...
    pub fn queue_start_command(&mut self, producer_id: ProcessId, command: OrchestratorCommand) {
        process_debug!(ProcessId::current(), "📋 Queuing start command for producer {}", producer_id);
