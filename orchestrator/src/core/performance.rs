...
impl PerformanceTracker {
    ...
    fn calculate_efficiency_metrics(&self) -> EfficiencyMetrics {
        ...
        for (provider_id, metrics) in &self.current_stats.by_provider {
            // Cost efficiency
            if best_cost_efficiency.is_none() || metrics.unique_per_dollar > best_cost_efficiency.unwrap().1 {
                best_cost_efficiency = Some((*provider_id, metrics.unique_per_dollar));
            }

            // Token efficiency
            if best_token_efficiency.is_none() || metrics.unique_per_1k_tokens > best_token_efficiency.unwrap().1 {
            ...
            total_efficiency_score +=
                metrics.unique_per_dollar * 0.4 + metrics.unique_per_1k_tokens * 0.3 + metrics.uam * 0.3;
        }
    ...
    fn calculate_trend_metrics(&self) -> TrendMetrics {
        ...
        TrendMetrics {
            ...
            efficiency_trend: self
                .calculate_trend_direction(first_metrics.unique_per_dollar, second_metrics.unique_per_dollar),
    ...
    fn calculate_stability_score(&self, first: &PerformanceMetrics, second: &PerformanceMetrics) -> f64 {
        ...
        let cost_variance = (first.cost_per_minute - second.cost_per_minute).abs()
            / ((first.cost_per_minute + second.cost_per_minute) / 2.0 + 0.001);
