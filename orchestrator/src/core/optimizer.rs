...
use shared::{ProviderId, RoutingStrategy, GenerationConfig, OptimizationMode};
...
pub enum OptimizationStrategy {
    ...
    Balanced { uam_weight: f64, cost_weight: f64, quality_weight: f64 },
...
impl Optimizer {
    ...
    pub fn optimize_for_topic(
        ...
        let partitioning = self.determine_partitioning_strategy(
            topic,
            current_performance,
        ...
        let optimized_prompt = self.generate_optimized_prompt(
            topic,
            &partitioning,
            &performance_analysis,
        )?;
        ...
        let routing_strategy = self.optimize_routing_strategy(
            current_performance,
            optimization_mode,
        )?;
        
        // Generate configuration adjustments
        let generation_config = self.optimize_generation_config(
            current_performance,
            optimization_mode,
        )?;
    ...
    fn generate_optimized_prompt(
        ...
        let base_prompt = match partitioning {
            PartitioningStrategy::None => {
                format!(
                    "Generate highly unique and specific attributes for '{}'. \
            ...
            PartitioningStrategy::ProviderSpecific { specializations: _ } => {
                ...
                format!(
                    "Generate unique attributes for '{}' leveraging your specific capabilities. \
        ...
        let enhanced_prompt = if analysis.overall_score < 0.5 {
            format!(
                "{}. IMPORTANT: Prioritize uniqueness over quantity - generate fewer but \
    ...
    fn optimize_routing_strategy(
        ...
        if performance.by_provider.len() == 1 && performance.overall.uam == 0.0 {
            let provider = performance.by_provider.keys().next().copied().unwrap_or(ProviderId::Random);
        ...
        match optimization_mode {
            OptimizationMode::MaximizeUAM { .. } => {
                ...
                let mut providers_by_uam: Vec<_> = performance.by_provider.iter()
            ...
            OptimizationMode::MinimizeCost { .. } => {
                ...
                let total_efficiency: f64 = performance.by_provider.values()
                    .map(|m| m.unique_per_dollar)
                    .sum();
            ...
            OptimizationMode::Weighted { uam_weight, cost_weight, .. } => {
    ...
    fn calculate_historical_success_rate(&self) -> f64 {
        ...
        let total_success: f64 = self.optimization_history.iter()
...
mod tests {
    use super::*;
    use crate::core::performance::{PerformanceStats, PerformanceMetrics};
    ...
    fn create_test_performance_stats() -> PerformanceStats {
        ...
        by_provider.insert(ProviderId::OpenAI, PerformanceMetrics {
            uam: 10.0,
            cost_per_minute: 0.5,
